...
namespace PrisonExtensions.WorkerGivers
{
    public class WorkGiver_Warden_DeliverFood : WorkGiver_Warden
    {
        public override Job JobOnThing(Pawn pawn, Thing t)
        {
            ...
            if ((double) prisoner.needs.food.CurLevelPercentage >= (double) prisoner.needs.food.PercentageThreshHungry +
                0.0199999995529652)
            {
            ...
            Thing foodSource;
            ThingDef foodDef;
            if (!FoodUtility.TryFindBestFoodSourceFor(pawn, prisoner,
                prisoner.needs.food.CurCategory == HungerCategory.Starving, out foodSource, out foodDef, false, true,
                false, false))
            {
                //Log.Message( string.Format( "\t{0} !TryFindBestFoodSourceFor", prisoner.LabelShort ) );
                return null;
            }
            if (prisoner.CanReach(foodSource, PathEndMode.ClosestTouch, prisoner.NormalMaxDanger(), false,
                TraverseMode.ByPawn))
            {
            ...
            job.count = FoodUtility.WillIngestStackCountOf(prisoner, foodDef);
        ...
        public static bool FoodAvailableInRoomTo(Pawn prisoner)
        {
            ...
            var room = prisoner.Position.GetRoom(Find.VisibleMap);
            ...
            for (int regionIndex = 0; regionIndex < room.RegionCount; ++regionIndex)
            {
                ...
                if (
                    (prisoner.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation)) && (foodSources.Any(
                        (source) =>
                        {
                            if (source.def.IsFoodDispenser)
                            {
                                ...
                                if (
                                    (source is Building_NutrientPasteDispenser) &&
                                    (((Building_NutrientPasteDispenser) source).HasEnoughFeedstockInHoppers())
                                )
                                {
                                    return true;
                                }
                ...
                for (int pawnIndex = 0; pawnIndex < pawns.Count; ++pawnIndex)
                {
                    ...
                    if (
                        ...
                        (pawn.needs.food.CurLevelPercentage <
                         (pawn.needs.food.PercentageThreshHungry + 0.0199999995529652)) &&
                        (
        ...
        public static float NutritionAvailableForFrom(Pawn p, Thing foodSource)
        {
            ...
            )
            {
                ...
                if (foodSource is Building_NutrientPasteDispenser)
                {
                    var FS = foodSource as Building_NutrientPasteDispenser;
                    var foodDef = FS.CanDispenseNow;
                    if (foodDef)
                    {
                        return 99999f;
                    }
